<script>
  ...
  import { getEntityHistory } from '../lib/api.js';
  ...
  onMount(async () => {
    try {
      ...
      if (history.length > 0 && history[0].domain) domain = history[0].domain;
    } catch { /* silent */ }
    loading = false;
  });
  ...
  function isOffState(state) {
    const s = String(state).toLowerCase();
    return s === 'off' || s === 'false' || s === 'not_home' || s === 'idle' || s === 'closed' || s === 'locked' || s === 'unavailable';
  }
</script>
...
<aside class="history-panel" role="dialog" aria-label="Entity history for {entityId}">
...
<style>
  ...
  @media (max-width: 480px) {
    ...
    .panel-header { padding: var(--sp-3) var(--sp-3); }
